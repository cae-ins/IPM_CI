...
*** Working Folder Path ***
global data "C:\Users\Dell\OneDrive\Bureau\PHAS\INPM\MICS\MICS 2016\Bases brutes" 	  
global sortie "C:\Users\Dell\OneDrive\Bureau\PHAS\INPM\MICS\MICS 2016\Sortie"
global dofile "C:\Users\Dell\OneDrive\Bureau\PHAS\INPM\MICS\MICS 2016\Dofile"
...
adopath + "C:\Users\Dell\OneDrive\Bureau\PHAS\INPM\MICS\who2007_update"


/* We use 'reflib' to specify the package directory where the .dta files 
containing the WHO Growth reference are stored. Note that we use strX to specify 
the length of the path in string. */		
gen str100 reflib = "C:\Users\Dell\OneDrive\Bureau\PHAS\INPM\MICS\who2007_update"
lab var reflib "Directory of reference tables"
...
gen str100 datalib = "C:\Users\Dell\OneDrive\Bureau\PHAS\INPM\MICS\who2007_update" 
lab var datalib "Directory for datafiles"
...
use "C:\Users\Dell\OneDrive\Bureau\PHAS\INPM\MICS\who2007_update/girl_nutri_civ_z.dta", clear 
...
save "$sortie/CIV16_WM_girls.dta", replace
	...
	//Erase files from folder:
erase "C:\Users\Dell\OneDrive\Bureau\PHAS\INPM\MICS\who2007_update//girl_nutri_civ_z.xls"
erase "C:\Users\Dell\OneDrive\Bureau\PHAS\INPM\MICS\who2007_update//girl_nutri_civ_prev.xls"
erase "C:\Users\Dell\OneDrive\Bureau\PHAS\INPM\MICS\who2007_update//girl_nutri_civ_z.dta"
...
*** Step 2.3 Literacy***
******************************************************************************** 
/*The entire household is considered deprived if 1 member aged 17 or over 
has not completed 8 years of education
 */ 
tab hl6 eduyears if hl6>=17
by hh_id, sort: gen literacy= (eduyears>=8 & hl6>=17)
lab var literacy "Un  membre du ménage de 17 ans ou plus ne sait pas lire ou écrire (Français) ; 1=oui, 0=non"
...
*** National MPI ***
codebook hc8a hc8b hc8c hc8d hc8e hc8f hc8g hc8h hc8i hc8j hc8k hc9a hc9b hc9c hc9d hc9e hc9f hc9g hc9h hc9i hc9j hc9l hc9m hc9n hc9o
...
egen n_small_assetsN = rowtotal(ventilator aircond television radio telephone refrigerator bicycle motorbike computer ), missing
lab var n_small_assetsN "Household Number of Small Assets Owned- National" 
   
gen hh_assetsN = (car==1 | n_small_assetsN > 1) 
replace hh_assetsN = . if car==. & n_small_assetsN==.
lab var hh_assetsN "Household Asset Ownership: HH has car or more than 1 small assets incl computer & animal cart"
...
codebook hl15a   
gen Identification = inlist(hl15a , 1, 2,3) 
lab var Identification "Household has a child who has a birth certificate, jugment or is declared"
 
save, replace
********************************************************************************
*** Step 2.11 Employment***
********************************************************************************
...
codebook mls5
gen dpH = mls7 == 0
keep hh1 hh2 ln mwb2 mls5 dpH
save "$sortie\MN.dta", replace

import spss using "$data\wm.sav", clear
rename _all, lower
gen ndpH = ls7 == 0
keep hh1 hh2 ln wb2 ls5 ndpH
ren wb2 mbw2
ren ls5 mls5
append using "$sortie\MN.dta"
gen double ind_id = hh1 *100000 + hh2 *100 + ln
save"$sortie\WM.dta", replace

gen cas= inrange( mwb2, 15, 24) & mls5!=1
by hh1 hh2, sort : egen touse = total(cas)
drop cas
gen cas = dpH & inrange( mwb2, 15, 24) & mls5!=1
by hh1 hh2, sort : egen tunemp = total(cas)
compare tunemp touse
gen d_unemp = tunemp==touse & tunemp!=0
sum d_unemp
drop touse cas tunemp dpH
sort hh1 hh2 ln
by hh1 hh2, sort: keep if _n==1
keep hh1 hh2 d_unemp
save "$sortie\Unemployment.dta", replace

use "$sortie\base_travail_mics2016.dta"
merge m:1 hh1 hh2 using "$sortie\Unemployment.dta"

save "$sortie\base_travail_mics2016.dta", replace

/* The household is deprived if a member does not have a birth certificate,
 supplementary judgment or is not declared */
...
recode literacy             (0=1)(1=0) , gen(d_literacy)
...
recode Identification       (0=1) (1=0), gen (id)
...
recode hh_mortality_u18_1y  (0=1)(1=0) , gen(d_cm1_01)
...
recode hh_years_edu_10      (0=1)(1=0) , gen(d_educ10_01)
recode liter                 (0=1)(1=0) , gen(d_literacy_01)
...
recode hh_assetsN 			(0=1)(1=0) , gen(d_asst1_01)
...
d_cm d_cm1 d_nutr d_satt d_educ d_educ10 d_literacy d_elct d_wtr d_sani d_hsg d_ckfl d_asst d_asst1 d_unemp Identification


order hh_id ind_id psu strata subsample weight ///
area region region_01 agec4 agec2 headship ///
d_cm d_cm1 d_nutr d_satt d_educ d_educ10 d_elct d_wtr d_sani d_hsg d_ckfl d_asst d_asst1 d_literacy d_unemp Identification
...
// PNUD
mpi d1(d_satt d_educ) w1(0.1666 0.1666) d2(d_cm d_nutr) w2(0.1666 0.1666) d3(d_elct d_wtr d_sani  d_hsg d_ckfl d_asst) w3(0.056 0.0556  0.0556 0.0556  0.0556  0.0556) [weight=weight]  ,cutoff(0.3333) by (region) 
...
putexcel clear
putexcel set  "$sortie\INPM_MICS_16_1", sheet("PNUD") replace
...
putexcel E5:M5 = "Contribution %"
putexcel N5:O5 = "% de la population"
putexcel P5:Q5 = "Milliers"
...
putexcel (D4:E4)="EDUCATION", merge
putexcel (F4:G4) ="SANTE"
putexcel (H4:M4)="CONDITIONS DE VIE", merge 
putexcel N4="Vulnérables à la pauvreté (qui connaissent une intensité de privations de 20 à 33,33 %)"
putexcel O4="En situation de pauvreté extrême (avec une intensité supérieure à 50 %)"
putexcel P4="POPULATION TOTALE"
putexcel Q4="POPULATION MULTIDIMENSIONELLEMENT PAUVRE"

putexcel (B3:M3)="PAUVRETE MULTIDIMENSIONELLE", merge

putexcel (P3:Q3)="POPULATION TOTALE", merge

putexcel (N4:Q4), bold border(bottom)
putexcel (B4:Q4), bold border(bottom)
putexcel (A2:U2), bold border(bottom)
putexcel (A40:U40), bold border(bottom)

putexcel (A19:Q19), bold border(bottom)
...
cap drop si vulnerable sev_pauvre poor
mpi d1(d_educ10 d_satt  d_literacy ) w1(0.083 0.083 0.083) d2(d_unemp) w2(0.25) d3(d_elct d_wtr d_sani  d_hsg d_ckfl  d_asst1) w3(0.041 0.041 0.041 0.041 0.041 0.041) d4( d_cm1) w4(0.25) [weight=weight]   , cutoff (0.3333) by (region) 
...
// Vulnerabilté
gen si = (d_educ10 + d_satt + d_literacy)*0.083 + d_unemp*0.25 + (d_elct + d_wtr + d_sani +  d_hsg + d_ckfl + d_asst)*0.041 + d_cm1*0.25
...
/* Exportation sur Excel dans le dossier Resultats_Tab*/
putexcel clear
putexcel set  "$sortie\INPM_MICS_16_1", sheet("Année de référence 1998") modify
...
putexcel C5 = "Borné entre O et 1"
putexcel D5 = "Contribution %"
putexcel E5:N5 = "Contribution %"
putexcel O5:P5 = "% de la population"
putexcel Q5:R5 = "Milliers"
...
putexcel O4="Vulnérables à la pauvreté (qui connaissent une intensité de privations de 20 à 33,33 %)"
putexcel P4="En situation de pauvreté extrême (avec une intensité supérieure à 50 %)"
putexcel Q4="POPULATION TOTALE"
putexcel R4="POPULATION MULTIDIMENSIONELLEMENT PAUVRE"

putexcel (B3:N3)="PAUVRETE MULTIDIMENSIONELLE", merge

putexcel (O3:R3)="POPULATION & VULNERABILITE", merge
putexcel (O4:R4), bold border(bottom)

putexcel (A2:R2), bold border(bottom)
putexcel (A19:R19), bold border(bottom)
...
mpi d1(d_educ10 d_satt  d_literacy ) w1(0.066 0.066 0.066) d2(d_unemp) w2(0.20) d3(d_elct d_wtr d_sani  d_hsg d_ckfl  d_asst1) w3(0.033 0.033 0.033 0.033 0.033 0.033) d4( d_cm1) w4(0.20) d5(id) w5(0.20) [weight=weight]  , cutoff(0.3333) by (region) 
...
// Vulnerabilté
gen si = (d_educ10 + d_satt + d_literacy)*0.066 + d_unemp*0.20 + (d_elct + d_wtr + d_sani +  d_hsg + d_ckfl + d_asst)*0.033 + d_cm1*0.20 + Identification*0.20
...
/* Exportation sur Excel dans le dossier Resultats_Tab*/
putexcel clear
putexcel set  "$sortie\INPM_MICS_16_1", sheet("Proposition Nationale") modify
...
mpi d1(d_satt d_educ) w1(0.17 0.17) d2(d_cm d_nutr) w2(0.17 0.16) d3(d_elct d_wtr d_sani  d_hsg d_ckfl d_asst) w3(0.056 0.0556  0.0556 0.0556  0.0556  0.0556) [weight=weight],cutoff(0.3333) by (area) 
...
/* Exportation sur Excel dans le dossier Resultats_Tab*/
putexcel clear
putexcel set  "$sortie\INPM_MICS_16_1", sheet("ZONE") modify
...
mat rownames A = `AR' 
putexcel A6 = "METHODE PNUD"

//year of reference 1998

cap drop si vulnerable sev_pauvre poor AR
mpi d1(d_educ10 d_satt  d_literacy ) w1(0.083 0.083 0.083) d2(d_cm1) w2(0.25) d3(d_elct d_wtr d_sani  d_hsg d_ckfl  d_asst1) w3(0.041 0.041 0.041 0.041 0.041 0.041) d4( d_unemp) w4(0.25) [weight=weight], cutoff (0.3333) by (area) 
...
*Vulnerabilté
gen si = (d_educ10 + d_satt + d_literacy)*0.083 + d_cm1*0.25 + (d_elct + d_wtr + d_sani +  d_hsg + d_ckfl + d_asst)*0.041 + d_unemp*0.25
...
putexcel clear
putexcel set  "$sortie\INPM_MICS_16_1", sheet("ZONE") modify
putexcel A10="Année de référence-1998"
...
mpi d1(d_educ10 d_satt  d_literacy ) w1(0.066 0.066 0.066) d2(d_cm1) w2(0.20) d3(d_elct d_wtr d_sani  d_hsg d_ckfl  d_asst1) w3(0.033 0.033 0.033 0.033 0.033 0.033) d4( d_unemp) w4(0.20) d5(id) w5(0.20) [weight=weight], cutoff(0.3333) by (area) 
* M0 et H
mat A = e(by_mpi)
mat list A
mat define A=e(by_mpi)'
...
// Vulnerabilté
gen si = (d_educ10 + d_satt + d_literacy)*0.066  + d_cm1*0.20 + (d_elct + d_wtr + d_sani +  d_hsg + d_ckfl + d_asst)*0.033  + d_unemp*0.20 + id*0.20
...
putexcel clear
putexcel set  "$sortie\INPM_MICS_16_1", sheet("ZONE") modify
putexcel A13="Proposition nationale"
