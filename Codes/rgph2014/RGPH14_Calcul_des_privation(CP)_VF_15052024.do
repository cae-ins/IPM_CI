...
gen edu = inlist(niveauinst , 3,4,5,6) & inrange( P19_AGE, 17,95) 
by  $list_var_menage, sort : egen nbedu = max(edu)
recode nbedu (0=1)(1=0), gen(educ)
replace educ=. if niveauinst==7
...
******ALPHABETISATION : 
*** Un  membre du ménage de 15 ans ou plus ne sait pas lire ou écrire (Français) 
gen alphab = 1 if (P28_ALPHABETISATION == 1) & inrange(P19_AGE, 15,49)
replace alphab = 0 if !(P28_ALPHABETISATION == 1)  & inrange(P19_AGE, 15,49)
replace alphab=. if P28_ALPHABETISATION==.
by $list_var_menage, sort: egen total_analph = max(alphab)
gen cas1 = inrange(P19_AGE, 15,49)
...
lab var solterre "Sol inadéquat; 1=oui, 0=non"
...
keep $list_var_menage desco educ mfsa chom P17_LIENPARENTE pasid  paselec paseaup  paselec paseaup combsale pasta solterre toiture matmur logement pasequi Ident educ1 DE59_DECES I07_NOMVILLAG I07_CODEVILLAG TOTMEN P28_ALPHABETISATION niveauinst P29_INSTRUCTION
