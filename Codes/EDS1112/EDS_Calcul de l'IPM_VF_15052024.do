...
*** Working Folder Path ***
global data "C:\Users\Dell\OneDrive\Bureau\PHAS\IPM-CI\EDS 1112\Bases brutes" 
global sortie "C:\Users\Dell\OneDrive\Bureau\PHAS\IPM-CI\EDS 1112\Sortie"
global dofile "C:\Users\Dell\OneDrive\Bureau\PHAS\IPM-CI\EDS 1112\Do file"
global adoigrowthup "C:\Users\Dell\OneDrive\Bureau\PHAS\IPM-CI\EDS 1112\Bases brutes\igrowup_update-master"
global adowho "C:\Users\Dell\OneDrive\Bureau\PHAS\IPM-CI\EDS 1112\Bases brutes\who2007_update"		
********************************************************************************
...
tab no_missing_edu10, miss
	//Check that values for 0 are less than 1%: 0.33%.
...
replace hh_years_edu_10 = . if hh_years_edu_10==0 & no_missing_edu10==0
...
*** National MPI ***
/*
The household does not have an improved toilet
*/
gen	toilet_mdg_1    = .
replace toilet_mdg_1 = (toilet<23 | toilet==41)
replace toilet_mdg_1 = 0 if !(toilet<23 | toilet==41)   
replace toilet_mdg_1 = . if toilet==.	
...
*** National MPI***
/*
The household is deprived if the nature of the wall, floor, earth is inadequate*/
...
*** National MPI ***
/*The household has at most 1 equipment from the following list:  television, radio, telephone, computer, refrigerator/freezer, 
bicycle, motorcycle, AND is deprived of a vehicle*/
...
keep mcaseid mv714a mv714 mv716 mv717 mv719 mv721 mv002 mv001 mv003 mv012 mv155 mv106
ren mcaseid hhid
ren mv714 v714
ren mv714a v714a
ren mv716 v716
...
// Literacy
/* The member is considered literate if he has a secondary or higher level, if he can read partially or entirely a sentence */
gen literacy= 1 if (v155==1|v155==2|v106==2|v106==3) & (v012>=15 & v012 <= 49)
replace literacy=0 if !(v155==1|v155==2|v106==2|v106==3) & (v012>=15 & v012 <= 49)
...
keep caseid v714a v714 v716 v717 v719 v721 v003 v002 v155 v001 v155 v012 v106
ren  caseid hhid
ren v001 hv001
ren v002 hv002
ren v003 hv003
...
bysort hhid : egen hh_employed = sum(employed) if employed!=.
gen empage = (hv105 >=15 & hv105 <=24)
gen temp = 1 if empage==1 & employed!=.
bysort hv002 hv003 : egen no_missing_emp_u = sum(temp) if  employed!=.
codebook hv121
...
lab var all_unemp "All 15-24 unemployed"
lab var some_unemp "At least one 15-24 unemployed"
clonevar d_unemp = some_unemp
by hhid, sort : gen first = _n==1

bysort hh_id: egen lit= max(literacy)
gen cas1 = v012>=15 & v012 <= 49
bysort hh_id: egen cas= total(cas1)
drop cas1
replace lit = 1 if cas == 0
drop cas
recode lit (0=1)(1=0), gen(d_lit)
lab var d_lit "A household member aged 15 or over does not know how to read (French); 1=yes, 0=no"
...
egen NBmis3 = rowmiss(d_satt d_educ1 d_lit d_elct d_wtr1 d_ckfl d_sani_1 d_hsg d_asst1 d_cm1 d_unemp Ident)
replace NBmis3 = 1 if NBmis !=0
recode NBmis3 (0=1) (1=0)
rename NBmis3 touse3
egen nbdeprN = rowtotal(d_satt d_educ1 d_lit d_elct d_wtr1 d_ckfl d_sani_1 d_hsg d_asst1 d_cm1 d_unemp Ident)
...
egen NBmis3 = rowmiss(d_satt d_educ d_lit d_elct d_wtr d_ckfl d_sani d_hsg d_asst d_cm d_unemp Ident)
replace NBmis3 = 1 if NBmis !=0
recode NBmis3 (0=1) (1=0)
rename NBmis3 touse3
egen nbdeprN = rowtotal(d_satt d_educ d_lit d_elct d_wtr d_ckfl d_sani d_hsg d_asst d_cm d_unemp Ident)
lab var nbdeprN "Number of deprivation Nat"
...
***********************************************************************MPI REGIONS ***********************************************************************
// UNDP NATIONAL THRESHOLD (includes UNDP indicators without nutrition mortality Drinking water)
...
// UNDP UNDP THRESHOLD (includes UNDP indicators without nutrition mortality Drinking water)
...
// UNDP NATIONAL THRESHOLD (includes basic UNDP indicators)
...
// UNDP UNDP THRESHOLD (includes basic UNDP indicators)
...
//UNDP NATIONAL THRESHOLD (includes standard indicators to which literacy and unemployment are added)
...
//  UNDP UNDP THRESHOLD (includes standard indicators to which literacy and unemployment are added)
mpi d1(d_educ d_satt d_lit ) w1(0.083 0.083 0.083)  d2(d_cm d_nutr) w2(0.125 0.125) d3(d_elct d_wtr d_sani  d_hsg d_ckfl  d_asst) w3(0.041 0.041 0.041 0.041 0.041 0.041) d4(d_unemp) w4(0.25) [pweight=weight] , cutoff (0.3333) by (region) 

// National Proposal_NATIONAL THRESHOLD (National proposal)

mpi d1(d_educ1 d_satt d_lit) w1(0.066 0.066 0.066)  d2(d_cm1) w2(0.2) d3(d_elct d_wtr1 d_sani_1  d_hsg d_ckfl  d_asst1) w3(0.033 0.033 0.033 0.033 0.033 0.033) d4(d_unemp) w4(0.2) d5(Ident) w5(0.2) [pweight=weight] , cutoff (0.3333) by (region) 
...
// Vulnerabilté
gen si = (d_educ1 + d_satt + d_lit)*0.066 + d_unemp*0.20 + (d_elct + d_wtr1 + d_sani +  d_hsg + d_ckfl + d_asst1)*0.033 + d_cm1*0.20 + Ident*0.20
...
putexcel clear
putexcel set "$sortie\IPM-CI_EDS_1112.xlsx", sheet("REGION_NATIONAL") modify
putexcel save

/* Mise en forme */
putexcel C6 = matrix(A), colnames  nformat("0.00")
putexcel A7 = matrix(A), rownames


//National Proposal_UNDP THRESHOLD (National proposal)
drop si vulnerable sev_pauvre poor R
mpi d1(d_educ d_satt  d_lit ) w1(0.066 0.066 0.066) d2(d_cm) w2(0.2) d3(d_elct d_wtr d_sani  d_hsg d_ckfl  d_asst) w3(0.033 0.033 0.033 0.033 0.033 0.033) d4( d_unemp) w4(0.20) d5(Ident) w5(0.20) [pweight=weight]  , cutoff(0.3333) by (region) 
...
putexcel clear
putexcel set  "$sortie\IPM-CI_EDS_1112", sheet("REGION_PNUD") modify
...
drop si vulnerable sev_pauvre poor R
mpi d1(d_educ1 d_satt  d_lit ) w1(0.066 0.066 0.066) d2(d_cm1) w2(0.20) d3(d_elct d_wtr1 d_sani_1  d_hsg d_ckfl  d_asst1) w3(0.033 0.033 0.033 0.033 0.033 0.033) d4(d_unemp) w4(0.20) d5(Ident) w5(0.20) [pweight=weight]  , cutoff(0.3333) by (area) 
...
// Vulnerabilté
gen si = (d_educ1 + d_satt + d_lit)*0.066 + d_cm1*0.20 + (d_elct + d_wtr1 + d_sani_1 +  d_hsg + d_ckfl + d_asst1)*0.033 + d_unemp*0.20 + Ident*0.20
...
/* Exportation sur Excel dans le dossier Resultats_Tab*/
putexcel clear
putexcel set  "$sortie\IPM-CI_EDS_1112", sheet("AREA_NATIONAL") modify
...
/* Exportation sur Excel dans le dossier Resultats_Tab*/
putexcel clear
putexcel set  "$sortie\IPM-CI_EDS_1112", sheet("AREA_PNUD") modify
