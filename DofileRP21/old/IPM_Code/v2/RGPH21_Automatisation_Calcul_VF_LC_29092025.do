...
foreach var of local   list_de_fichier{
   do "$do\RGPH21_Calcul_de_lIPM_VF_LC_29092025.do" `var' `reg'
   local reg = `reg' + 1
}
...
putexcel clear
putexcel set  "$out\IPM-CI_LC_2021", sheet("LOCALITE") modify
...
clear
import excel "$out\IPM-CI_LC_2021", sheet("LOCALITE") firstrow
gen SPID=real(B)
drop A B 
sort SPID
order SPID
export excel using "$out\IPM-CI_SP_2021", replace sheet("LOCALITE") firstrow(variables)  

save "$out\IPM-CI_LC_2021", replace
