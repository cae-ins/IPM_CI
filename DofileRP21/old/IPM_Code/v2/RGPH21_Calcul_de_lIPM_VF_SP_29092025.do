...
bysort SOUSPREFID: egen total_w = total(TOTMEN)
*preserve 
...
drop if missing(TOTMEN) | TOTMEN == 0
...
mpi d1(desco educ mfsa) w1(0.066 0.066 0.066) d2(chom) w2(0.2) d3(paselec paseaup combsale  pasta logement pasequi) w3( 0.033 0.033 0.033 0.033 0.033 0.033)d4(mjuv) w4(0.2) d5(Ident) w5(0.2) [fw=TOTMEN],cutoff(0.3333) by(SOUSPREFID)
...
ge vulnerable = (si> 0.2) & (si < 0.3333) if si!=.

gen sev_pauvre = si >= 0.5 if si!=.

gen poor = si >= 0.3333  if si!=.

proportion SOUSPREFID [fw=TOTMEN] if si!=., over(vulnerable)
mat list r(table)
...
proportion SOUSPREFID [fw=TOTMEN] if si!=., over(sev_pauvre)
mat list r(table)
mat define temp = e(b)'
...
// Population
qui tab SOUSPREFID [fw=TOTMEN] if si!=.,matcell(pop)

tab SOUSPREFID poor [fw=TOTMEN] if si!=.& poor==1,matcell(poor)
