...
* ---- Collapse INDIVIDUEL ----
collapse (mean) desco educ mfsa  chom   Ident [pw=TOTMEN_w],by(SOUSPREFID) 
/*	
...
save "$out\priv_indiv_sp.dta", replace
...
* Garder les résidents habituels
keep if P15D == 1
keep if P16 == 1
...
/**********************************************************************
   Collapse MENAGE -> SP
**********************************************************************/
collapse (mean) electricite eau_potable energie_cuisson toilette ///
         logement equipement mjuv, ///
    by(SOUSPREFID)

save "$out\priv_menage_sp.dta", replace


/**********************************************************************
    4. MERGE INDIVIDUEL + MENAGE
**********************************************************************/
use "$out\priv_indiv_sp.dta", clear
merge 1:1 SOUSPREFID using "$out\priv_menage_sp.dta"
drop _merge



save "$out\Sortie\privations_SP_final.dta", replace

export excel using "$out\IPM-CI_2021_vf_13112025.xlsx" ,  sheet("PRIVATION_SP") sheetmodify firstrow(variable)
