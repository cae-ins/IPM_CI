...
bysort P06: egen total_w = total(TAILLE_MENAGE)
*preserve 
*duplicates drop   P06, force
*br REGION P06 total_w TAILLE_MENAGE if total_w == 0
...
mpi d1(desco educ mfsa) w1(0.066 0.066 0.066) d2(chom) w2(0.2) d3(paselec paseaup combsale  pasta logement pasequi) w3( 0.033 0.033 0.033 0.033 0.033 0.033)d4(mjuv) w4(0.2) d5(Ident) w5(0.2) [fw=TAILLE_MENAGE],cutoff(0.3333) by(P06)
...
mat A_reg_`code_region'  = H_Sp,M_Sp,Indicateur_reg, vulnerable,sev_pauvre

// On enlève temporairement la variables poor car on a des localités sans ménages pauvres. On reflechira après de quoi faire. Pareil pour pop vulnerable,sev_pauvre, pop
...
mat colnames A_reg_`code_region' = "H" "MO" "Frequentation scolaire"  "Année de scolarité" "Alphabétisation" "Chomage" "Electricité" "Eau potable" "Energie de cuisson"  "Toilette" "Logement"  "Equipement" "Mortalité juvénile" "Déclaration d'état civil" "Vulnerabilité" "sev_pauvre" 
