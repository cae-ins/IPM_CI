...
\begin{itemize}
  \item \textbf{65,8~\%} de la population ivoirienne vit dans un ménage multidimensionnellement
        pauvre, soit 19,6 millions de personnes.
  \item Ces personnes cumulent en moyenne \textbf{47,4~\%} des privations pondérées possibles.
  \item L'indice qui combine les deux vaut $\ipm = \mathbf{0{,}3122}$.
  \item Le milieu rural rassemble \textbf{63,3~\%} de la pauvreté pour 47,3~\% de la population.
  \item L'écart entre régions va de 0,132 (Abidjan) à 0,468 (Bounkani), soit un rapport de
        un à trois et demi.
\end{itemize}
\end{retenir}
...
égales entre ses indicateurs : un indicateur seul dans sa dimension porte 0,25 à lui seul, quatre
indicateurs se partagent 0,0625 chacun.
...
Santé & 2 & 0,1250 & 0,25 \\
Emploi & 1 & 0,2500 & 0,25 \\
Conditions de vie & 6 & 0,0417 & 0,25 \\
\midrule
\textbf{Ensemble} & \textbf{13} & & \textbf{1,00} \\
\bottomrule
\end{tabular}
\end{center}
...
On part d'un tableau $X$ à 12\,965 lignes (les ménages) et 13 colonnes (les indicateurs), qui
...
Le vecteur $z = (z_1, \dots, z_{13})$ donne le seuil de chaque indicateur. Il ne prend pas
...
\[
c_i \;=\; \sum_{j=1}^{13} w_j \, g^0_{ij}, \qquad 0 \le c_i \le 1 .
\]
...
\textbf{0,3968} et le maximum observé de 0,9583 : aucun ménage n'est privé sur les treize
...
ne comptent pas dans l'indice. Sur l'EHCVM 2021, la censure efface \textbf{14,8~\%} des privations
pondérées, celles de 3\,386 ménages.
...
\begin{tabular}{@{}lrl@{}}
\toprule
\textbf{Indice} & \textbf{Valeur} & \textbf{Lecture} \\
\midrule
$H$ --- incidence & 0,6584 & 65,8~\% de la population est pauvre, soit 19,6 millions de personnes \\
$A$ --- intensité & 0,4742 & les pauvres cumulent en moyenne 47,4~\% des privations pondérées \\
\ipm{} & 0,3122 & $= H \times A$ \\
Vulnérables & 19,1~\% & score entre 0,2 et 1/3 \\
Pauvreté sévère & 26,8~\% & score d'au moins 0,5 \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
Urbain & 52,7~\% & 48,0~\% & 0,4531 & 0,2175 \\
Rural  & 47,3~\% & 85,7~\% & 0,4874 & 0,4178 \\
\midrule
Chef de ménage homme & 83,6~\% & 67,0~\% & 0,4751 & 0,3183 \\
Chef de ménage femme & 16,4~\% & 59,9~\% & 0,4693 & 0,2810 \\
\bottomrule
\end{tabular}
\end{center}

Le rural concentre \textbf{63,3~\%} de la pauvreté multidimensionnelle avec 47,3~\% de la
population ; son \ipm{} est presque le double de l'urbain. Entre régions, l'écart va de 0,132
(Abidjan) à 0,468 (Bounkani). Quatre régions --- Béré, Folon, Worodougou, Bounkani --- dépassent
90~\% d'incidence.
...
Les quatre dimensions sont équipondérées \emph{en droit}, mais elles pèsent en fait de 4,6~\% à
40,5~\% --- un facteur neuf. Le poids $w_j$ fixe le maximum qu'une dimension \emph{peut}
apporter ; ce qu'elle apporte \emph{vraiment} dépend de la fréquence de la privation. Le chômage
détient le poids individuel le plus élevé du système (0,25, seul dans sa dimension) et la
contribution la plus faible, parce qu'il ne touche que 5,7~\% des pauvres. Une dimension à un seul
indicateur est toujours à la merci de la fréquence de cet indicateur : trop fréquent, il domine ;
trop rare, il disparaît. Les deux dimensions à indicateur unique sont précisément celles qui
s'écartent le plus de leur poids nominal.
\end{attention}
...
\begin{description}
  ...
  Santé repose donc sur l'assurance maladie et l'insécurité alimentaire. Ce dernier indicateur
...
\col{preconstruction\_...} (12\,965 $\times$ 29) \\
02 & applique le vecteur $z$ &
\col{vecteur\_z}, \col{matrice\_situationnelle\_...} (13 indicateurs 0/1) \\
...
05 & calcule $H$, $A$, \ipm{}, contributions, désagrégations &
...
\begin{verbatim}
python pipeline/orchestrateur.py            # les cinq étapes, environ 4 secondes
