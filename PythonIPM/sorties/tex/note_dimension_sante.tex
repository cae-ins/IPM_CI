...
arithmétique en cause, documente les décisions prises --- l'ajout de l'insécurité alimentaire mesurée
par l'échelle FIES de la FAO, puis celui du renoncement aux soins pour raison de coût ou
d'indisponibilité de l'offre --- et en
mesure l'effet. Elle se termine sur ce qui reste ouvert.
\end{abstract}
...
\section{Le troisième indicateur : le renoncement aux soins}

Un \textbf{troisième indicateur} a ensuite été ajouté à la dimension : un ménage est privé si l'un
de ses membres a eu un problème de santé dans les 30 derniers jours (question 3.01), n'a
\emph{pas} consulté (3.05) et a cité comme raison principale (3.06) soit le \textbf{coût}
(« trop cher », « manque d'argent »), soit l'\textbf{indisponibilité de l'offre} de soins
(« service spécialisé non disponible », « absence de personnel »).

Ces quatre modalités ont en commun d'être \textbf{subies}. Elles s'opposent aux motifs qui
relèvent d'un arbitrage du ménage --- l'automédication, le « pas nécessaire », la peur du
résultat --- lesquels ne rendent pas privé : renoncer parce qu'on juge la consultation inutile
n'est pas la même chose que renoncer parce qu'on n'en a pas les moyens ou qu'il n'y a personne
pour soigner.

Là où l'assurance maladie mesure une \emph{couverture} formelle --- largement absente en Côte
d'Ivoire, donc peu discriminante --- le renoncement aux soins mesure un \textbf{accès effectif
empêché}, par l'argent ou par l'absence de service. C'est un fait, pas un droit théorique : le
ménage a été malade, et n'a pas pu se soigner. L'indicateur prive \textbf{9,3~\% des ménages}
(9,0~\% de la population). Il ne peut être privé que parmi les 67,2~\% de ménages ayant connu un
problème de santé sur la période : un ménage où personne n'a été malade n'est pas privé.

\begin{retenir}[Le coût domine très largement]
Les deux motifs d'offre sont rares : sur les 1\,720 personnes qui renoncent, 1\,706 le font pour
une raison d'argent et 14 seulement pour indisponibilité du service ou absence de personnel. Les
inclure n'ajoute que 10 ménages privés sur 12\,965 et déplace l'\ipm{} de 0,000079. Le choix
relève donc de la \emph{définition} --- un renoncement subi reste un renoncement subi, quelle
qu'en soit la cause --- et non d'un effet sur les résultats.
\end{retenir}

\section{L'effet mesuré}

La dimension Santé compte désormais trois indicateurs à \textbf{0,0833} chacun : le poids de
l'assurance maladie est le tiers de ce qu'il était.

\begin{center}
\small
\begin{tabular}{@{}lrrr@{}}
\toprule
& \textbf{Avant} & \textbf{Étape 2} & \textbf{Aujourd'hui} \\
& \small(assurance seule) & \small(+ FIES) & \small(+ renoncement) \\
\midrule
Indicateurs de l'IPM & 13 & 13 & 16 \\
Poids de l'assurance maladie & 0,2500 & 0,1250 & 0,0833 \\
Part de l'assurance dans le score moyen & 50,8~\% & 29,4~\% & 19,9~\% \\
Score moyen $c_i$ & 0,4603 & 0,3968 & 0,3918 \\
\textbf{$H$ --- incidence} & \textbf{79,7~\%} & \textbf{65,8~\%} & \textbf{58,0~\%} \\
Vulnérables & 12,7~\% & 19,1~\% & 22,8~\% \\
Pauvreté sévère & 44,0~\% & 26,8~\% & 26,1~\% \\
Ménages au score exactement égal à $k$ & 396 & 928 & 16 \\
\bottomrule
\end{tabular}
\end{center}

\emph{La colonne « Aujourd'hui » n'isole pas le seul effet du renoncement aux soins : elle intègre
les trois indicateurs ajoutés en même temps (renoncement, emploi agricole de subsistance,
promiscuité), qui font passer l'IPM de treize à seize indicateurs.}

L'effet est net sur la pauvreté sévère, qui passe de 44,0~\% à 26,1~\% : c'est là que la
domination d'un indicateur unique se voyait le plus. La dimension Santé, qui pesait 40,5~\% de
l'\ipm{} pour un poids nominal de 25~\%, en pèse désormais \textbf{27,9~\%} --- l'assurance
maladie 16,7~\%, l'insécurité alimentaire 9,0~\%, le renoncement aux soins 2,2~\%.
...
\begin{attention}[$H = 58{,}0~\%$ demeure au-dessus de l'ordre de grandeur de référence]
L'assurance maladie pèse encore \textbf{16,7~\% de l'IPM} --- la deuxième contribution de tout le
système --- pour un indicateur qui prive 93~\% des ménages. Trois leviers restent disponibles, non
tranchés à ce jour.
\end{attention}

\begin{enumerate}
  \item \textbf{Abandonner l'assurance maladie} et garder le FIES et le renoncement aux soins en
  Santé. Cohérent avec le constat qu'un indicateur à 93~\% n'informe pas ; la dimension garderait
  cette fois deux jambes, ce qui n'était pas le cas au moment du premier arbitrage.
  ...
  ménages, et posent la même question de pouvoir discriminant. Le cas s'est reproduit hors Santé :
  l'emploi agricole de subsistance prive 48,1~\% des ménages et apporte à lui seul 16,9~\% de
  l'\ipm{}, contre 1,6~\% au chômage, à poids identique.
\end{enumerate}
